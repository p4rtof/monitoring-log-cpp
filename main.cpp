...
struct Log { // Bikin cetakan data (struct) buat satu baris log
...
void bacaDataFile(string filename) { // Fungsi buat baca data dari file pas awal jalan
    ifstream file(filename); // Buka file txt-nya
    if (!file.is_open()) return; // Kalau file nggak ada, yaudah batalin (keluar fungsi)

    string line; // Siapin variabel buat nyimpen teks per baris
    while (getline(file, line)) { // Looping: baca file baris demi baris sampai habis
        if (line.empty()) continue; // Kalau barisnya kosong, lewatin aja
...
void cariPakaiHashTable(string targetLevel) { // Fungsi nyari data pakai Hash Table (Khusus Level)
...
void tambahLog() { 
    ...
    cout << "ID: "; cin >> baru.id; cin.ignore(); // cin.ignore() biar enter-nya nggak ngaco ke input selanjutnya
...
void cariLog(int pilihan, string keyword) {
    ...
    for (const auto& log : listLog) {
        ...
        if (pilihan == 1 && log.waktu.find(keyword) != string::npos) cocok = true; // Nyari waktu (bisa sebagian kata)
        else if (pilihan == 2 && log.level == keyword) cocok = true; // Nyari level (harus sama persis)
        else if (pilihan == 3 && log.sumber == keyword) cocok = true; // Nyari sumber (harus sama persis)
    ...
    // Mulai nampilin data ke layar 
    cout << "\n--- Hasil Pencarian ---\n";
    if (!hasilCari.empty()) { // Kalau wadah sementaranya nggak kosong
        for (const auto& log : hasilCari) { // Print semua isinya
            cout << "[" << log.waktu << "] " << log.level << " | " << log.sumber << " : " << log.pesan << "\n";
        }
...
int main() {
    ...
    while (true) { // Looping terus-terusan sampai user milih Keluar
        // Tampilan menu utama
        ...
        if (menu == 5) { // Kalau pilih 5, program berhenti
            cout << "Program selesai. Babay!\n";
            break; // Keluar dari looping while
        }

        if (menu == 1) { // Kalau pilih 1, panggil fungsi tambah
            tambahLog();
        } else if (menu == 3) { 
            ...
            if (menu == 2) cariLog(1, cari); // 1 untuk nyari Waktu
            if (menu == 4) cariLog(3, cari); // 3 untuk nyari Sumber
